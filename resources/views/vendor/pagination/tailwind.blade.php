...
<p class="text-sm text-gray-700 leading-5">
    @if ($paginator->total() > 0)
        {{ $paginator->firstItem() }}～{{ $paginator->lastItem() }}項目目（{{ $paginator->total() }}項目中）
    @else
        0件
    @endif
</p>
    ...
    <span aria-current="page">
        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-800 bg-green-300 border border-green-500 cursor-default leading-5 rounded-md shadow-sm">
            {{ $page }}
        </span>
    </span>
                                @else
